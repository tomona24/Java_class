...
public class DataTypes {
    public static void main(String[] args) {
        ...
        short b = 2020;  // 16 bits int (-2^15 to 2^15)
        int c = 1000;   // 32 bits (-2^31 to 2^31)
        long d = 12301L;  // 64 bits (-2^63 to  - 2^63)

        char e = 'A';  // 8 bits - single character (with single quote)
        ...
        // Increment(Decrement): ++, --
        int x = 10;
        int y = ++x;  // y = 11, prefix (increment and assign　+1してからx出す)
        int z = x++;  // z = 11, postfix (assign and increment　x出して、次回+1する。この時点は11)
        System.out.println(x);  // 次回になるため、x は12になっている。
    }
